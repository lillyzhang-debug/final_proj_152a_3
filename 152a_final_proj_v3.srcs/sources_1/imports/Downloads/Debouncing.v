...
//        stable_btnR <= 1'b0;
    ...
    deb_sw15[0] <= sync_sw15[1];
    deb_sw15[1] <= deb_sw15[0];
    
    deb_sw14[0] <= sync_sw14[1];
    deb_sw14[1] <= deb_sw14[0];

	deb_sw13[0] <= sync_sw13[1];
	deb_sw13[1] <= deb_sw13[0];
    
	deb_btnS[0] <= sync_btnS[1];
	deb_btnS[1] <= deb_btnS[0];
    
    deb_btnA[0] <= sync_btnA[1];
    deb_btnA[1] <= deb_btnA[0];

	deb_btnB[0] <= sync_btnB[1];
    deb_btnB[1] <= deb_btnB[0];

	deb_btnC[0] <= sync_btnC[1];
    deb_btnC[1] <= deb_btnC[0];

	deb_btnD[0] <= sync_btnD[1];
